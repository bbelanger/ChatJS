...
namespace ChatJsMvcSample.Code.LongPolling.Chat
{
    public class ChatLongPollingProvider : LongPollingProvider
    {
        ...
        public override IEnumerable<LongPollingEvent> WaitForEvents(int myUserId, string roomId, long timestamp, string connectionString, Controller controller)
        {
            ...
            // just return them
            var existingMessages = LongPollingChatServer.Rooms[roomId].GetMessagesAfter(timestamp).Where(shouldMessageBeSentToCurrentUser).ToList();
            ...
            if (LongPollingChatServer.Rooms[roomId].HasChangedSince(timestamp))
                events.Add(GetLongPollingEventForRoomUsersChanged(LongPollingChatServer.Rooms[roomId].UpdateStatusesAndGetUserList(), myUserId));

            // updates the user last active on
            LongPollingChatServer.Rooms[roomId].UpdateLastActiveOn(myUserId);
            ...
            if (!events.Any())
            {
                ...
                using (LongPollingChatServer.Rooms[roomId].SubscribeForUsersChange(
                        (r, users) =>
                        {
                ...
                using (LongPollingChatServer.Rooms[roomId].SubscribeForMessagesChange(
                ...
                using (LongPollingChatServer.Rooms[roomId].SubscribeForUserTyped(
                    (s, userFrom, userTo) =>
                    {
